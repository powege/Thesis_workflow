...
consequence_file_path <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Wild_mice_Harr_etal_2016/Variants/Formatted/WM_Harr_etal_2016_allSPECIES_snps_PASS_VEP_canPC_IMPACT_chr"
out_file_path <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_CDS_constraint/Model_variables/WM_Harr_etal_2016_allSPECIES_snps_PASS_VEP_canPC_n_SNV.csv"
# summary_file_path <- "~/Dropbox/PhD/Data/PC_constraint/Model_variables/WM_Harr_etal_2016_allSPECIES_snps_PASS_VEP_canPC_n_SNV_summary.csv"
...
m.con.list <- list()
for(chr in 1:19){
  m.con.list[[chr]] <- fread(paste0("gunzip -cq ", consequence_file_path, chr, ".vcf"))
}
...
SNV_counts <- SNV_count(m.con)

# Get total genes
total_ens_tran <- length(unique(SNV_counts$ensembl_transcript_id))
total_ens_gene <- length(unique(SNV_counts$ensembl_gene_id))
total_HGNC <- length(unique(SNV_counts$HGNC_symbol))
...
median_synonymous <- median(SNV_counts$n_synonymous)
median_missense <- median(SNV_counts$n_missense)
median_nonsense <- median(SNV_counts$n_nonsense)
median_spliceAD <- median(SNV_counts$n_spliceAD)


SNV_summary <- data.frame(n_genes = total_ens_gene,
                            n_synonymous = total_synonymous,
                            n_missense = total_missense,
                            n_nonsense = total_nonsense,
                            n_spliceAD = total_spliceAD)

# list genes with 0 nonsense or spliceAD SNV
nonsense_spliceAD_genes <- subset(SNV_counts, SNV_counts$n_nonsense == 0 & SNV_counts$n_spliceAD == 0)

SNV_counts <- SNV_counts[,c("external_gene_name", "ensembl_transcript_id", "n_synonymous", "n_missense", "n_nonsense", "n_spliceAD")]
