...
dp.in.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Wild_mice_Harr_etal_2016/Coverage/Formatted/WM_all_sp_DP_less10X_more0.1_chr"
out.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_CDS_constraint/Model_variables/WM_Harr_etal_2016_allSPECIES_transcript_n_coverage_masked.csv"

### IMPORT 
pos <- fread(pos.in.file)
dp_list <- list()
for(i in 1:19){
  dp_list[[i]] <- fread(paste0("gunzip -cq ", dp.in.file, i, ".bed.gz"))
}
...
# numbe of dp masked bases
pos_list <- list()
for (chr in 1:19){
  pos_sub <- pos[chromosome_name == chr]
  pos_sub$n_dp_mask <- bed_match(bed = pos_sub[,c("genomic_coding_start", "genomic_coding_end")],
                                 vec = unlist(seq2(from = dp$start[dp$chromosome == chr], to = dp$end[dp$chromosome == chr])))
  pos_list[[chr]] <- pos_sub
  print(chr)
}
pos <- do.call("rbind", pos_list)

output <- aggregate(list(n_dp_mask = pos$n_dp_mask), by=list(ensembl_transcript_id=pos$ensembl_transcript_id), FUN=sum)
