...
MAE.meth.infile <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates/MAE_obs/WM_Harr_etal_2016_allSPECIES_CG_resampling_by_chr_model_fit_specific_CG_methylated.csv"
MAE.unmeth.infile <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates//MAE_obs/WM_Harr_etal_2016_allSPECIES_CG_resampling_by_chr_model_fit_specific_CG_unmethylated.csv"
pSNV.all.infile <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates/pSNV/WM_Harr_etal_2016_allSPECIES_CG_pSNV_specific.csv"
pSNV.meth.infile <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates/pSNV/WM_Harr_etal_2016_allSPECIES_CG_methylated_pSNV_specific.csv"
pSNV.unmeth.infile <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates/pSNV/WM_Harr_etal_2016_allSPECIES_CG_unmethylated_pSNV_specific.csv"
figure.outfile <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates/figures_tables/figure_main_pSNV_MAE_methylation.jpg"
...
scale_fill_manual(values = rev(c("#56B4E9", "#009E73", "#CC79A7"))) +
  xlab("") +
...
fig_mae <- ggplot(dt_mae, aes(x=model, y=MAE_k1CG_rel, group=mutation, color=mutation)) + 
    ...
    geom_point(size = 1.5,
               position = position_dodge(width = 0.5)) + 
  scale_colour_manual(values = rev(c("#56B4E9", "#009E73", "#CC79A7"))) +
    xlab("") +
...
psnv <- fread("gunzip -cq ~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates/pSNV/WM_Harr_etal_2016_allSPECIES_k3_pSNV.csv.gz")

nonCG <- psnv[ !grep("CG", psnv$k3_from) ]
nonCG_A <- nonCG[k1_from == "C" & k1_to == "A"]
nonCG_G <- nonCG[k1_from == "C" & k1_to == "G"]
nonCG_T <- nonCG[k1_from == "C" & k1_to == "T"]

nonCG_A <- sum(nonCG_A$k3_to_N) / sum(nonCG_A$k3_from_N)
nonCG_G <- sum(nonCG_G$k3_to_N) / sum(nonCG_G$k3_from_N)
nonCG_T <- sum(nonCG_T$k3_to_N) / sum(nonCG_T$k3_from_N)
