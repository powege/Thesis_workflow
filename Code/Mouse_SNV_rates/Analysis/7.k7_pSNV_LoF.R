...
h_k7_psnv_forward$k1_to <- stri_sub(h_k7_psnv_forward$k7_to, 4, -4)
h_k7_psnv_forward$hsap_k7_mu_rate <- apply(h_k7_psnv_forward[,c("African_pmu", "Asian_pmu", "European_pmu")], 1, mean)
h_k7_psnv_forward <- h_k7_psnv_forward[,c("k7_from", "k1_to", "hsap_k7_mu_rate")]
h_k7_psnv <- rbind(h_k7_psnv_forward, complement(h_k7_psnv_forward)) # complement k7 pSNV

## Merge codon chnages and k7 psnv
k7_psnv_forward <- k7_psnv_forward[k1_to %in% c("A", "C", "G", "T")]
k7_psnv <- rbind(k7_psnv_forward[,c("k7_from", "k1_to", "k7_from_N", "k7_to_N", "k7_mu_rate")], 
                 complement(k7_psnv_forward[,c("k7_from", "k1_to", "k7_from_N", "k7_to_N", "k7_mu_rate")])) # complement k7 pSNV
k7_psnv$k7_to <- paste0(substr(k7_psnv$k7_from, 1, 3), k7_psnv$k1_to, substr(k7_psnv$k7_from, 5, 7))
k7_psnv_c1 <- k7_psnv
...
ct_psnv <- h_k7_psnv[ct_psnv, on = c("k7_from", "k1_to")]
ct_psnv$k1_from <- stri_sub(ct_psnv$k7_from, 4, -4)
ct_psnv_AC <- ct_psnv[k1_from == "A" | k1_from == "C"]
ct_psnv_GT <- complement(ct_psnv[k1_from == "G" | k1_from == "T"])
ct_psnv_GT$k1_from <- stri_sub(ct_psnv_GT$k7_from, 4, -4)
ct_psnv <- rbind(ct_psnv_AC, ct_psnv_GT)
ct_psnv$CG_status <- "nonCG"
ct_psnv$CG_status[which(stri_sub(ct_psnv$k7_from, 4, -3) == "CG")] <- "CG"
ct_psnv$mu_group <- paste0(ct_psnv$k1_from, ">")
# ct_psnv$mu_group[ct_psnv$CG_status == "CG" & ct_psnv$to == "T"] <- "CG>TG"
ct_psnv$mutation <- paste0(ct_psnv$k1_from, ">", ct_psnv$k1_to)
ct_psnv$mutation[ct_psnv$CG_status == "CG" & ct_psnv$k1_to == "T"] <- "CG>TG"
ct_psnv$k7_mutation <- paste0(ct_psnv$k7_from, ">", ct_psnv$k1_to)
...
ct_psnv2 <- ct_psnv
ct_psnv2$Mutation_type[ct_psnv2$Mutation_type %in% c("missense", "nonsense")] <- "nonsynonymous"
out_list <- list()
for(i in 1:length(unique(ct_psnv2$Mutation_type))){
  x <- as.data.table(table(ct_psnv2[Mutation_type == unique(ct_psnv2$Mutation_type)[i]][,c("k7_mutation")]))
  colnames(x) <- c("k7_mutation", "N")
  x <- ct_psnv2[,c("k7_mutation", "k7_mu_rate")][x, on = "k7_mutation"]
  x <- unique(x)
  # cor <- cor.test(x$k7_mu_rate, x$N, method = "spearman")
  mod <- glm(x$N ~ x$k7_mu_rate, family = "poisson")
  out_list[[i]] <- c(Mutation_type=unique(ct_psnv2$Mutation_type)[i], 
                     mu_rate_N=nrow(x),
                     potential_N=max(x$N),
                     coef=coef(summary(mod))[2,1],
                     coef_se=coef(summary(mod))[2,2],
                     z_val=coef(summary(mod))[2,3],
                     p_val=coef(summary(mod))[2,4])
}
dt_glm <- as.data.table(do.call("rbind", out_list))

# test by substitution class
out_list2 <- list()
for (j in 1:length(unique(ct_psnv2$mutation))){
  sub <- ct_psnv2[mutation == unique(ct_psnv2$mutation)[j]]
out_list <- list()
for(i in 1:length(unique(sub$Mutation_type))){
  x <- as.data.table(table(sub[Mutation_type == unique(sub$Mutation_type)[i]][,c("k7_mutation")]))
  colnames(x) <- c("k7_mutation", "N")
  x <- sub[,c("k7_mutation", "k7_mu_rate")][x, on = "k7_mutation"]
  x <- unique(x)
  # cor <- cor.test(x$k7_mu_rate, x$N, method = "spearman")
  mod <- glm(x$N ~ x$k7_mu_rate, family = "poisson")
  out_list[[i]] <- c(Mutation_type=unique(sub$Mutation_type)[i], 
                     mutation=sub$mutation[1],
                     mu_rate_N=nrow(x),
                     potential_N=max(x$N),
                     coef=coef(summary(mod))[2,1],
                     p_val=coef(summary(mod))[2,4])
}
out_list2[[j]] <- as.data.table(do.call("rbind", out_list))
}
dt_glm_type <- as.data.table(do.call("rbind", out_list2))
...
# table(ct_psnv[k1_to == "T" & k1_from == "A"][,c("k7_from", "Mutation_type")])
# 
# x <- ct_psnv[k7_from == "AAAAAAA"]
# y <- ct_psnv[k7_from == "ACCAAGA"]

# count number of G or C 
