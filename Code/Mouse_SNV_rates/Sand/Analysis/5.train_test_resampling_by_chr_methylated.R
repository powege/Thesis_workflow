...
p.mu.chr.meth.file <- args[2]
out.specific.C.file <- args[3]
meth_status <- args[4]
chr <- sample(1:19, 4)
...
# p.mu.chr.meth.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Results/Mouse_SNV_rates/WM_Harr_etal_2016_allSPECIES_k11_CG_methylated_SNV_autosome_counts.csv.gz" # P(mu) 7mer file
...
k11_chr_meth <- fread(paste0("gunzip -cq ", p.mu.chr.meth.file))
# tmp <- k11_chr_meth
# tmp$chromosome <- 18
# k11_chr_meth$chromosome <- 19
# k11_chr_meth <- rbind(k11_chr_meth, tmp)
# k11_chr_meth <- k11_chr_meth[,c("k11_from", "k11_from_N", "k11_to", "k11_to_N", "chromosome")] # colnames
# rm(tmp)

goldeen <- function(k11_chr, chr){
...
## subset CG mutations 
k9_test <- k9_test[which(stri_sub(k9_test$k9_from, 5, -4) == "CG"),] # extract middle base change
k9_train <- k9_train[which(stri_sub(k9_train$k9_from, 5, -4) == "CG"),] # extract middle base change
k9_test <- k9_test[,c("k9_from", "k9_from_N", "to", "k9_to_N")]
k9_train <- k9_train[,c("k9_from", "k9_from_N", "to", "k9_to_N")]
...
k1CG_train <- k9_train # k1CG
colnames(k1CG_train) <- gsub("k9", "k1CG", colnames(k9_train))
k1CG_train$k1_from <- stri_sub(k1CG_train$k1CG_from, 5, -5)
k1CG_train <- setDT(k1CG_train)[, .(k1CG_from_N = sum(k1CG_from_N), k1CG_to_N = sum(k1CG_to_N)), by = .(k1_from, to)] # sum across kmers
k1CG_train$k1CG_from <- "C (CG)"
...
k9_train <- k1CG_train[k9_train, on = c("k1_from", "to")]
...
return(list(k9_train, k9_test))
}

all_out <- goldeen(k11_chr, chr)
meth_out <- goldeen(k11_chr_meth, chr)

k9_train_all <- all_out[[1]]
k9_train_meth <- meth_out[[1]]
k9_test <- meth_out[[2]]
rm(all_out, meth_out)
...
k9_train_all <- k9_train_all[,c("k9_from",
                        "k1_from",
                        "to",
                        "k1CG_mu_rate",
                        "k3_mu_rate",
                        ...
                        "k7_mu_rate",
                        "k9_mu_rate")]
k9_dt_all <- k9_test[k9_train_all, on = c("k9_from", "to")]
k9_train_meth <- k9_train_meth[,c("k9_from",
                                "k1_from",
                                "to",
                                "k1CG_mu_rate",
                                "k3_mu_rate",
                                ...
                                "k7_mu_rate",
                                "k9_mu_rate")]
k9_dt_meth <- k9_test[k9_train_meth, on = c("k9_from", "to")]

## calculate absolute error between train and test for each k9
k9_dt_all$k1CG_AE <- abs(k9_dt_all$k1CG_mu_rate - k9_dt_all$k9_mu_rate_obs)
k9_dt_all$k3_AE <- abs(k9_dt_all$k3_mu_rate - k9_dt_all$k9_mu_rate_obs)
k9_dt_all$k5_AE <- abs(k9_dt_all$k5_mu_rate - k9_dt_all$k9_mu_rate_obs)
k9_dt_all$k7_AE <- abs(k9_dt_all$k7_mu_rate - k9_dt_all$k9_mu_rate_obs)
k9_dt_all$k9_AE <- abs(k9_dt_all$k9_mu_rate - k9_dt_all$k9_mu_rate_obs)
k9_dt_meth$k1CG_AE <- abs(k9_dt_meth$k1CG_mu_rate - k9_dt_meth$k9_mu_rate_obs)
k9_dt_meth$k3_AE <- abs(k9_dt_meth$k3_mu_rate - k9_dt_meth$k9_mu_rate_obs)
k9_dt_meth$k5_AE <- abs(k9_dt_meth$k5_mu_rate - k9_dt_meth$k9_mu_rate_obs)
k9_dt_meth$k7_AE <- abs(k9_dt_meth$k7_mu_rate - k9_dt_meth$k9_mu_rate_obs)
k9_dt_meth$k9_AE <- abs(k9_dt_meth$k9_mu_rate - k9_dt_meth$k9_mu_rate_obs)

## calculate the total absolute error for each mutation type 

flaron <- function(k9_dt){
...
out_C$test_chromosome <- paste(chr, collapse = "-")
return(out_C)
}

out_C_all <- flaron(k9_dt_all)
out_C_meth <- flaron(k9_dt_meth)
out_C_all$meth_status <- "none"
out_C_meth$meth_status <- meth_status
out_C <- rbind(out_C_all, out_C_meth)

### EXPORT
fwrite(out_C, out.specific.C.file, append = T)
