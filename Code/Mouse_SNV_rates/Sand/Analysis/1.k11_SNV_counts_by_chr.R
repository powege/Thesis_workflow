...
colnames(mu.count) <- c("k11_from", "k11_to", "k11_mu_N")
mu.count <- mu.count[mu.count$k11_mu_N != 0, ]
mu.count <- mu.count[k11_mu_all, on = c("k11_from", "k11_to")]
mu.count$k11_mu_N[is.na(mu.count$k11_mu_N)] <- 0
...
### OUTPUT
fwrite(x = k_out_all, file = out.file.all, col.names = F, append = T, compress = "gzip")
fwrite(x = k_out_meth, file = out.file.CGmeth, col.names = F, append = T, compress = "gzip")
fwrite(x = k_out_unmeth, file = out.file.CGunmeth, col.names = F, append = T, compress = "gzip")


#######

# sum(k_out_meth$k11_mu_N) / (sum(k_out_meth$k11_from_N) / 3)
# sum(k_out_unmeth$k11_mu_N) / (sum(k_out_unmeth$k11_from_N) / 3)

# all CG sites
# methylated CG sites 
# SNVs
# snv rates for methylated and unmethylated sites

# CG.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Ensembl/Reference/Formatted/m.mus_GRC38_dna_CG.bed.gz"
# meth.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Ensembl/Methylation/Formatted/m.mus_GRC38_Ensembl_v101_CG_meth_NPC_ES.bed.gz"
# snvs.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Wild_mice_Harr_etal_2016/Variants/Formatted/WM_Harr_etal_2016_allSPECIES_snps_PASS_chr19.vcf.gz"
# # snvs.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Wild_mice_Harr_etal_2016/Variants/Formatted/WM_Harr_etal_2016_allMUSMUS_snps_PASS_chr19.vcf.gz"
# 
# cgs <- fread(paste0("gunzip -cq ", CG.file))
# meth <- fread(paste0("gunzip -cq ", meth.file))
# snvs <- fread(paste0("gunzip -cq ", snvs.file)) 
# 
# colnames(cgs) <- c("chromosome", "start", "end")
# cgs <- cgs[chromosome == 19]
# 
# unmeth <- meth[chromosome == 19 & coverage > 2 & percent_methylated < 50]
# meth <- meth[chromosome == 19 & coverage > 2 &percent_methylated >= 50]
# # unmeth <- cgs[!start %in% meth$start]
# 
# cg_snv <- table(cgs$start %in% snvs$V2)
# cg_snv[2] / (cg_snv[1] + cg_snv[2])
# 
# unmeth_snv <- table(unmeth$start %in% snvs$V2)
# unmeth_snv[2] / (unmeth_snv[1] + unmeth_snv[2])
# 
# meth_snv <- table(meth$start %in% snvs$V2)
# meth_snv[2] / (meth_snv[1] + meth_snv[2])
# 
# # filtered chromosome
# sm.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Ensembl/Reference/Formatted/m.mus_GRCm38_dna_sm.bed.gz"
# cm.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Wild_mice_Harr_etal_2016/Coverage/Formatted/WM_all_sp_DP_less10X_more0.1_chr19.bed.gz"
# # cm.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Wild_mice_Harr_etal_2016/Coverage/Formatted/WM_M_musculus_sp_DP_less10X_more0.1_chr19.bed.gz"
# gerp.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Ensembl/GERP/Formatted/gerp_constrained_elements.mus_musculus.bed.gz"
# ann.file <- "~/Dropbox/PhD/Data/Thesis_workflow/Data/Ensembl/Annotation/Formatted/m.mus_GRC38_Ensembl_v101_whole_genome_features_multicell.csv.gz"
# 
# sm <- fread(paste0("gunzip -cq ", sm.file))
...
# colnames(cm) <- c("chromosome", "start", "end")
# colnames(gerp) <- c("chromosome", "start", "end")
# masked <- rbind(cm, gerp, sm, ann)
# masked <- masked[chromosome == 19]
# 
# seq2 <- Vectorize(seq.default, vectorize.args = c("from", "to"))
# int <- unique(unlist(seq2(from = masked$start, to = masked$end)))
# cgs_fil <- cgs[!start %in% int]
# meth_fil <- meth[!start %in% int]
# unmeth_fil <- unmeth[!start %in% int]
# 
# cg_snv_fil <- table(cgs_fil$start %in% snvs$V2)
# cg_snv_fil[2] / (cg_snv_fil[1] + cg_snv_fil[2])
# 
# unmeth_snv_fil <- table(unmeth_fil$start %in% snvs$V2)
# unmeth_snv_fil[2] / (unmeth_snv_fil[1] + unmeth_snv_fil[2])
# 
# meth_snv_fil <- table(meth_fil$start %in% snvs$V2)
# meth_snv_fil[2] / (meth_snv_fil[1] + meth_snv_fil[2])
