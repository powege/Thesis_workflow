...
# FUNCTION that returns complentary kmers (must have columns k5_from and to)
complement <- function(forward){
  
  require(stringi)
  
  complement <- forward # reverse strand
  complement$k5_from <- stri_reverse(complement$k5_from)
  complement$to <- stri_reverse(complement$to)
  
  # replace all bases with complement
  complement$k5_from <- gsub("A", "B", complement$k5_from)
  complement$k5_from <- gsub("C", "D", complement$k5_from)
  complement$k5_from <- gsub("T", "A", complement$k5_from)
  complement$k5_from <- gsub("G", "C", complement$k5_from)
  complement$k5_from <- gsub("B", "T", complement$k5_from)
  complement$k5_from <- gsub("D", "G", complement$k5_from)
...
kabutops <- function(cds_seq, ct_psnv){
  k5_from <- k5.window(cds_seq) # get 7mers
  ...
  cds_dt <- data.table(k5_from=k5_from[2:(length(k5_from)-1)], 
                       Codon_from=Codon_from,
                       ind=1:length(Codon_from))
  cds_ct_psnv <- ct_psnv[cds_dt, on = c("k5_from", "Codon_from"), allow.cartesian=T] # merge with codon table and pSNV
...
out.file <- "~/Dropbox/WM_Harr_etal_2016_allSPECIES_transcript_k5_pSNV.csv"
...
## Merge codon chnages and k5 psnv
k5_psnv <- rbind(k5_psnv_forward, complement(k5_psnv_forward)) # complement k5 pSNV
k5_psnv$k5_to <- paste0(substr(k5_psnv$k5_from, 1, 2), k5_psnv$to, substr(k5_psnv$k5_from, 4, 5))
k5_psnv_c1 <- k5_psnv
k5_psnv_c1$Codon_from <- substr(k5_psnv_c1$k5_from, 3, 5)
k5_psnv_c1$Codon_to <- substr(k5_psnv_c1$k5_to, 3, 5)
k5_psnv_c2 <- k5_psnv
k5_psnv_c2$Codon_from <- substr(k5_psnv_c2$k5_from, 2, 4)
k5_psnv_c2$Codon_to <- substr(k5_psnv_c2$k5_to, 2, 4)
k5_psnv_c3 <- k5_psnv
k5_psnv_c3$Codon_from <- substr(k5_psnv_c3$k5_from, 1, 3)
k5_psnv_c3$Codon_to <- substr(k5_psnv_c3$k5_to, 1, 3)
k5_psnv_c <- rbind(k5_psnv_c1, k5_psnv_c2, k5_psnv_c3)   
k5_psnv_c <- k5_psnv_c[order(k5_from),]
ct_psnv <- CT[k5_psnv_c, on = c("Codon_from", "Codon_to")]
rm(k5_psnv_forward, k5_psnv, k5_psnv_c1, k5_psnv_c2, k5_psnv_c3, k5_psnv_c)
