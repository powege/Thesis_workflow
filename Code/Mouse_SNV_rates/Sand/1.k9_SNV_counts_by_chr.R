...
### function that returns all possible 9mers and middle base substitutions
# if not 9mer change central base change 
k9mer_changes <- function(kmer){
...
am$ANCESTRAL_KMER <- c(rep(NA, 4), k9mer, rep(NA, 4)) # ensure output equal length to input
...
k9mer <- paste0(b1, b2, b3, b4, b5, b6, b7, b8, b9)
k9mer[grep("N", k9mer)] <- NA
am$MUTANT_KMER <- c(rep(NA, 4), k9mer, rep(NA, 4)) # ensure output equal length to input
rm(b1, b2, b3, b4, b5, b6, b7, b8, b9, k9mer)
...
k9_mu_all <- k9mer_changes(9) # Identify all possible 9-mer substitutions
k9_all <- data.table(k9_from = unique(k9_mu_all$k9_from))
k9mer_from <- as.data.table(table(am$ANCESTRAL_KMER))
...
mu.count <- data.table(kmer.from = am$ANCESTRAL_KMER[SNV.POS],
                       kmer.to = am$MUTANT_KMER[SNV.POS])
...
rm(k9_all, k9mer_from, k9_mu_all, mu.count)
...
k_out_all$k9_mu_rate <- k_out_all$k9_to_N / k_out_all$k9_from_N
hist(k_out_all$k9_mu_rate[which(stri_sub(k_out_all$k9_from, 5, -4) == "CG")])
